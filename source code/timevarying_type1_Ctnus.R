...
Timevarying_gnrt_ctn <- function(N = 200, Distribution = "Exponential", censor.rate = 1){
	...
	Data$X2 <- as.vector(rbind(runif(N,0,5), runif(N,5,10))) 
	...
	if(Distribution == "Exponential"){
		...
		while(Count <= N){
			...
			if( -log(u) < Lamda*exp(Beta1*(x1>=1))*t0 ){ ## Lamda*exp(Beta1*(x1>1))*t0
				Data[Data$ID==Count,][1,]$Start = 0
				T = -log(u)/(Lamda*exp(Beta1*(x1>=1))) ## Lamda*exp(Beta1*(x1>1))
				...
				T = (-log(u)-Lamda*exp(Beta1*(x1>=1))*t0+Lamda*exp(Beta1*(x1>=1)+Beta2)*t0)/(Lamda*exp(Beta1*(x1>=1)+Beta2)) ##replace x1 by (x1>1)
		...
		while(Count <= N){
			...
			if( -log(u) < Lamda*exp(Beta1*(x1>=1))*t0^V ){ # replace x1 by (x1>1)
				Data[Data$ID==Count,][1,]$Start = 0
				T = (-log(u)/(Lamda*exp(Beta1*(x1>=1))))^(1/V) # replace x1 by (x1>1)
				Data[Data$ID==Count,][1,]$Stop = T
				Data[Data$ID==Count,][1,]$Event = 1
				...
				Data[Data$ID==Count,][2,]$Start = t0
				T1 = (-log(u)-Lamda*exp(Beta1*(x1>=1))*t0^V+Lamda*exp(Beta1*(x1>=1))*exp(Beta2)*t0^V)/(Lamda*exp(Beta1*(x1>=1))*exp(Beta2)) # replace x1 by (x1>1)
				T = T1^(1/V)
				Data[Data$ID==Count,][2,]$Stop = T
				Data[Data$ID==Count,][2,]$Event = 1
			}
		...
		while(Count <= N){
			...
			if( -log(u) < Lamda*exp(Beta1*(x1>=1))/Alpha*(exp(Alpha*t0)-1) ){ # replace x1 by (x1>1)
				Data[Data$ID==Count,][1,]$Start = 0
				T1 = 1+Alpha*(-log(u))/(Lamda*exp(Beta1*(x1>=1))) # replace x1 by (x1>1)
				T = (1/Alpha)*log(T1)
				Data[Data$ID==Count,][1,]$Stop = T
				Data[Data$ID==Count,][1,]$Event = 1
				...
				Data[Data$ID==Count,][2,]$Start = t0
				T1 = Alpha*(-log(u))/(Lamda*exp(Beta1*(x1>=1)+Beta2))-(exp(Alpha*t0)-1-exp(Beta2+Alpha*t0))/exp(Beta2) # replace x1 by (x1>1)
				T = (1/Alpha)*log(T1)
				Data[Data$ID==Count,][2,]$Stop = T
				Data[Data$ID==Count,][2,]$Event = 1
			}
...
Timevarying_gnrt_ctn_test <- function(N = 200, Distribution = "Exponential"){
