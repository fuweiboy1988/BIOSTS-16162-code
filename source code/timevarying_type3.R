...
Range_T <- function(T1, T2, T3, DIST, X, U){
	x1 = X
	u = U
	if( !(T1 < T2 && T2 < T3) ){
			TTT <- sort(c(T1,T2,T3)) 
...
###================================================================
Timevarying_gnrt3 <- function(N = 200, Distribution = "Exponential", censor.rate = 1){
	...
	while(Count <= N){
		...
		#	Ts = runif(3,0.6,6) ## original runif(3,0.7,3.5)
		...
		RT <- Range_T(t1,t2,t3,Distribution,x,u)
		Time = RT$T
		Row.ID = RT$Row
...
##======================================================================================
Timevarying_gnrt3_test <- function(N = 200, Distribution = "Exponential"){
